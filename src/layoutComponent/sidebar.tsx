...
const SideBar = () => {
  ...
  const topMenuItems: MenuItem[] = [
    { icon: HiMenuAlt2, name: "Dashboard" },
    { icon: BsCalendarCheck, name: "Reservation" },
    { icon: FaBed, name: "Rooms" },
    { icon: FaCheckCircle, name: "Checks" },
    { icon: FaWallet, name: "Billing" },
  ];
...
<div className="pl-6 pt-4.5"> 
    ...
    <ul className="flex flex-col">
        {topMenuItems.map(({ icon: Icon, name }, index) => (
        <li
            key={index}
            title={name}
      ...
      <ul className="flex flex-col mt-auto gap-4 pb-5">
        {bottomMenuItems.map(({ icon: Icon, name }, index) => (
          <li
            key={index}
            title={name}
