...
const SideBar = () => {
  ...
  return (
    <div className="pr-2 pt-1.5">
      <div
        className={` flex flex-col  h-[99vh] bg-[#3b2f2f] text-gray-100 p-3 rounded-2xl font-roboto transition-all duration-300 overflow-hidden ${
        ...
        <ul className="flex flex-col">
          {topMenuItems.map(({ icon: Icon, name, path }, index) => (
            <li key={index} title={name}>
              ...
              >
                <Icon
                  className={`${isOpen ? "text-2xl" : "text-1xl"}
              transition-all duration-300`}
                />
                {isOpen && (
                  <span className="ml-2 text-[16px] whitespace-nowrap">
                    {name}
                  </span>
                )}
              </Link>
