...
const SideBar = () => {
  ...
  //  Move handleLogout inside component
  const navigate = useNavigate();
  const handleLogout = async () => {
    try {
      await logout();
       navigate("/", { replace: true });// redirection au  login
    } catch (error) {
  ...
  return (
    <div className="pl-1 pt-2">
      <div
        className={`flex flex-col h-[95vh] bg-[#3b2f2f] text-gray-100 p-3 rounded-2xl font-roboto transition-all duration-300 overflow-hidden ${
      ...
      >
          ...
          {topMenuItems.map(({ icon: Icon, name, path }, index) => (
            <li key={index} title={name}>
              ...
              >
                ...
                />
                {isOpen && (
                  <span className="ml-2 text-[16px] whitespace-nowrap">{name}</span>
                )}
              </Link>
            </li>
          ))}
        ...
        <ul className="flex flex-col mt-auto gap-4 pb-5">
          {bottomMenuItems.map(({ icon: Icon, name, path, action }, index) => (
            <li key={index} title={name}>
              {action ? (
                // For Logout or custom actions
                <button
                  onClick={action}
                  className="flex w-full items-center justify-start gap-2 h-[50px] px-3 text-lg hover:bg-[#E9E6E1] hover:text-[#3b2f2f] rounded-md cursor-pointer transition-all duration-300"
                >
                  <Icon
                    className={`${
                      isOpen ? "text-2xl" : "text-1xl"
